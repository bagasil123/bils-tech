<x-layouts.admin title="Edit Project">
    ...
    <div class="max-w-2xl">
        <form
            ...
            class="space-y-5"
            x-data="imagePreview('{{ $project->image ? Storage::url($project->image) : '' }}')"
        >
            @csrf
            @method('PATCH')
            ...
            <div class="card-paper p-5">
                <h2 class="font-serif text-base font-semibold text-ink mb-4">Gambar Thumbnail</h2>
                <div class="space-y-3">
                    <div class="relative w-full aspect-video bg-paper-200 border border-warm-border rounded-sm overflow-hidden flex items-center justify-center">
                        <img x-show="previewUrl" :src="previewUrl" class="absolute inset-0 w-full h-full object-cover" />
                        <div x-show="!previewUrl" class="text-center">
                            <svg class="w-10 h-10 text-warm-border mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                        </div>
                    </div>
                    ...
                    <p class="text-xs text-warm-gray">Kosongkan jika tidak ingin mengganti gambar. JPG, PNG, WebP — maks. 3MB</p>
            ...
            <div class="card-paper p-5 space-y-5">
                ...
                <div>
                    ...
                    <input id="demo_link" type="url" name="demo_link" value="{{ old('demo_link', $project->demo_link) }}"
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4"
                              class="form-textarea @error('description') border-red-400 @enderror">{{ old('description', $project->description) }}</textarea>
                    @error('description') <p class="form-error">{{ $message }}</p> @enderror
                </div>
            </div>

            <div class="flex gap-3">
                <button type="submit" class="btn-primary">Perbarui Project</button>
